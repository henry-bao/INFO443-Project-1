...
describe("NavBar", () => {
  test("Navagation bar renders without crashing", () => {
    render(
      <BrowserRouter>
        <NavBar isMobile={false} signInStatus={false} statusWord="Sign in" />
      </BrowserRouter>
    );
  ...
  test("Sign in and Sign out buttons have the corrent href", () => {
    ...
    render(
      <BrowserRouter>
        <NavBar isMobile={false} signInStatus={true} statusWord={"Sign out"} />
      </BrowserRouter>
    );
  ...
  test("Burger menu only shows up when isMobile is true", () => {
    const mobileComponent = shallow(
      <NavBar isMobile={true} signInStatus={false} statusWord={"Sign in"} />
    );
    expect(mobileComponent.text().includes("Burger")).toBeTruthy();

    const nonMobileComponent = shallow(
      <NavBar isMobile={false} signInStatus={false} statusWord={"Sign in"} />
    );
  ...
  test("Buger menu renders correctly", () => {
    render(
      <BrowserRouter>
        <Burger signInStatus={false} statusWord={"Sign in"} />
      </BrowserRouter>
    );
  });
});
